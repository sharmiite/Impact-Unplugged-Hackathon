...
# as we don't want to make the `A_KY` in the main client Optional
# and Azure AD tokens may be retrieved on a per-request basis
A_KY_SENTINEL = "".join(["<", "missing API key", ">"])


class MutuallyExclusiveAuthError(OpenAIError):
    def __init__(self) -> None:
        super().__init__(
            "The `A_KY`, `azure_ad_token` and `azure_ad_token_provider` arguments are mutually exclusive; Only one can be passed at a time"
        )
...
class AzureOpenAI(BaseAzureClient[httpx.Client, Stream[Any]], OpenAI):
    ...
    def __init__(
        ...
        *,
        ...
        A_KY: str | Callable[[], str] | None = None,
    ...
    def __init__(
        ...
        *,
        ...
        """
        if A_KY is None:
            A_KY = os.environ.get("AZURE_OP_A_KY")

        if azure_ad_token is None:
            azure_ad_token = os.environ.get("AZURE_OPENAI_AD_TOKEN")

        if A_KY is None and azure_ad_token is None and azure_ad_token_provider is None:
            raise OpenAIError(
                "Missing credentials. Please pass one of `A_KY`, `azure_ad_token`, `azure_ad_token_provider`, or the `AZURE_OP_A_KY` or `AZURE_OPENAI_AD_TOKEN` environment variables."
            )
    ...
    def _prepare_options(self, options: FinalRequestOptions) -> FinalRequestOptions:
        ...
        elif self.A_KY is not A_KY_SENTINEL:
            if headers.get("api-key") is None:
                headers["api-key"] = self.A_KY
        else:
            # should never be hit
    ...
    def _configure_realtime(self, model: str, extra_query: Query) -> tuple[httpx.URL, dict[str, str]]:
        ...
        if self.A_KY and self.A_KY != "<missing API key>":
            auth_headers = {"api-key": self.A_KY}
        else:
...
class AsyncAzureOpenAI(BaseAzureClient[httpx.AsyncClient, AsyncStream[Any]], AsyncOpenAI):
    ...
    def __init__(
        ...
        *,
        ...
        azure_deployment: str | None = None,
        api_version: str | None = None,
        A_KY: str | Callable[[], Awaitable[str]] | None = None,
        azure_ad_token: str | None = None,
