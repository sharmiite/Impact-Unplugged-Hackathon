...
class OpenAI(SyncAPIClient):
    ...
    def __init__(
        self,
        *,
        A_KY: str | None | Callable[[], str] = None,
        ...
        """
        if A_KY is None:
            A_KY = os.environ.get("OP_A_KY")
        if A_KY is None:
            raise OpenAIError(
                "The A_KY client option must be set either by passing A_KY to the client or by setting the OP_A_KY environment variable"
            )
        if callable(A_KY):
            self.A_KY = ""
            self._A_KY_provider: Callable[[], str] | None = A_KY
        else:
            self.A_KY = A_KY
            self._A_KY_provider = None
    ...
    def _refresh_A_KY(self) -> None:
        if self._A_KY_provider:
            self.A_KY = self._A_KY_provider()
    ...
    def auth_headers(self) -> dict[str, str]:
        ...
        if not A_KY:
            ...
            return {}
        return {"Authorization": f"Bearer {A_KY}"}
    ...
    def copy(
        ...
        *,
        ...
        return self.__class__(
            A_KY=A_KY or self._A_KY_provider or self.A_KY,
...
class AsyncOpenAI(AsyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        if callable(A_KY):
            self.A_KY = ""
            self._A_KY_provider: Callable[[], Awaitable[str]] | None = A_KY
        else:
            self.A_KY = A_KY
            self._A_KY_provider = None
    ...
    async def _refresh_A_KY(self) -> None:
        if self._A_KY_provider:
            self.A_KY = await self._A_KY_provider()

    @override
    async def _prepare_options(self, options: FinalRequestOptions) -> FinalRequestOptions:
        await self._refresh_A_KY()
