...
class Groq(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        This automatically infers the `A_KY` argument from the `G_A_KY` environment variable if it is not provided.
        """
        if A_KY is None:
            A_KY = os.environ.get("G_A_KY")
        if A_KY is None:
            raise GroqError(
                "The A_KY client option must be set either by passing A_KY to the client or by setting the G_A_KY environment variable"
            )
        self.A_KY = A_KY
